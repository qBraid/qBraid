...
# Copyright (C) 2021 The Cirq Developers
#
...
def test_rotation_gates_wrong_number_of_args(qasm_gate: str):
    ...
    with pytest.raises(QasmException, match=rf".*{qasm_gate}.* takes 1.*got.*2.*line 5"):
        parser.parse(qasm)
...
def test_rotation_gates_zero_params_error(qasm_gate: str):
    ...
    with pytest.raises(QasmException, match=rf".*{qasm_gate}.* takes 1.*got.*0.*line 4"):
        parser.parse(qasm)
...
def test_standard_gates_wrong_params_error(qasm_gate: str, num_params: int):
    ...
    with pytest.raises(QasmException, match=rf".*{qasm_gate}.* takes {num_params}.*got.*5.*line 4"):
        parser.parse(qasm)
    ...
    with pytest.raises(QasmException, match=rf".*{qasm_gate}.* takes {num_params}.*got.*0.*line 4"):
        parser.parse(qasm)
...
def test_three_qubit_gates_not_enough_args(qasm_gate: str):
    ...
    with pytest.raises(
        QasmException, match=rf""".*{qasm_gate}.* takes 3 arg\(s\).*got.*1.*line 4"""
    ):
        parser.parse(qasm)
