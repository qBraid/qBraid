...
# pylint: disable=import-outside-toplevel,implicit-str-concat
...
@pytest.mark.skipif(not HAS_QISKIT_PKG or qiskit_to_qat is None, reason="Extra not installed")
...
@pytest.mark.skipif(not HAS_QISKIT_PKG or qat_to_qiskit is None, reason="Extra not installed")
...
@pytest.mark.skipif(not HAS_CIRQ_PKG or cirq_to_qat is None, reason="Extra not installed")
...
def test_qat_node_in_default_graph():
    ...
    if not has_extra(qasm2_to_qat):
        pytest.skip("myqlm-interop not installed")
    graph = ConversionGraph()
    assert "qat" in graph.nodes()
...
def test_qiskit_to_qat_requires_extras_attribute():
    ...
    if not HAS_QISKIT_PKG or qiskit_to_qat is None:
        pytest.skip("qiskit package not available")
...
def test_cirq_to_qat_requires_extras_attribute():
    ...
    if not HAS_CIRQ_PKG or cirq_to_qat is None:
        pytest.skip("cirq package not available")
