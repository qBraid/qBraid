...
def test_qasm_qubits():
    ...
    assert load_program(qasm3_bell()).qubits == [("q", 2)]
    assert load_program(qasm3_shared15()).qubits == [("q", 4)]
...
__bits__[0] = measure __qubits__[0];"""
    assert load_program(qasm3_str).depth == 1
...
def test_qasm3_random(num_qubits, depth, max_operands, seed, measure):
    ...
    assert load_program(circuit).num_qubits >= 1
    if num_qubits is not None:
        assert load_program(circuit).num_qubits == num_qubits
...
def test_qasm3_random_with_known_seed():
    ...
    assert load_program(circuit).num_qubits == 3
...
def test_populate_idle_qubits_qasm3():
    ...
    qprogram = load_program(qasm_test)
