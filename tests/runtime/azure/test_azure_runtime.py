...
def test_serialize_pulser_input():
    ...
    ``serialize_pulser_input`` only wraps pulser's own ``to_abstract_repr()`` under a
    ``sequence_builder`` key, so that envelope is what this asserts. Pinning the full
    serialized device spec instead would re-test pulser's schema, which changes on its
    own release schedule -- pulser 1.9 altered ``AnalogDevice`` and broke this test
    without anything in qBraid changing.
    """
    pytest.importorskip("pulser", reason="Pasqal pulser package is not installed.")
    ...
    payload = json.loads(pulser_input)
    assert set(payload) == {"sequence_builder"}

    builder = payload["sequence_builder"]
    assert builder == json.loads(sequence.to_abstract_repr())
    assert builder["register"] == [{"name": "q0", "x": 0.0, "y": 0.0}]
    assert builder["device"]["name"] == "AnalogDevice"
    assert builder["pulser_version"] == pulser.__version__
