...
from qbraid import random_circuit, to_unitary
from qbraid.transpiler2.interface import convert_to_cirq, convert_from_cirq, transpile, CircuitConversionError
...
cirq_circuit_braket, braket_unitary = braket_shared_gates_circuit()
cirq_circuit_cirq, cirq_unitary = cirq_shared_gates_circuit()
cirq_circuit_qiskit, qiskit_unitary = qiskit_shared_gates_circuit()
...
data_test_shared_gates = [
    (cirq_circuit_braket, "cirq", cirq_unitary),
    (cirq_circuit_braket, "qiskit", qiskit_unitary),
    (cirq_circuit_qiskit, "braket", braket_unitary),
    (cirq_circuit_qiskit, "cirq", cirq_unitary),
    (cirq_circuit_cirq, "braket", braket_unitary),
    (cirq_circuit_cirq, "qiskit", qiskit_unitary),
]


@pytest.mark.parametrize("cirq_circuit,target_package,target_unitary", data_test_shared_gates)
def test_shared_gates(cirq_circuit, target_package, target_unitary):
    ...
    """
    transpiled_circuit = convert_from_cirq(cirq_circuit, target_package)
...
def test_gate_intersect_braket_qiskit(gate_str):
    if gate_str in ["SXdg", "iSwap", "RYY", "RZZ", "CPhase", "RXX", "Phase"]:
        assert True
    else:
        ...
        braket_circuit_transpile = transpile(qbraid_qiskit_circ, "braket")
        qiskit_circuit_transpile = transpile(qbraid_braket_circ, "qiskit")
        braket_transpile_u = to_unitary(braket_circuit_transpile)
        qiskit_transpile_u = to_unitary(qiskit_circuit_transpile)
        # assert np.allclose(braket_u, braket_transpile_u)  # TODO
...
def test_gate_intersect_qiskit_cirq(gate_str):
    if gate_str in ["Phase", "CPhase", "iSwap", "Swap"]:
        assert True
    else:
        qiskit_init_gate = qiskit_gates[gate_str]
        nqubits, nparams = nqubits_nparams(gate_str)
        ...
        qiskit_circuit_transpile = transpile(qbraid_cirq_circ, "qiskit")
        cirq_circuit_transpile = transpile(qbraid_qiskit_circ, "cirq")
        qiskit_transpile_u = to_unitary(qiskit_circuit_transpile)
...
def test_gate_intersect_braket_cirq(gate_str):
    ...
    braket_circuit_transpile = transpile(qbraid_cirq_circ, "braket")
    cirq_circuit_transpile = transpile(qbraid_braket_circ, "cirq")
    braket_transpile_u = to_unitary(braket_circuit_transpile)
    cirq_transpile_u = to_unitary(cirq_circuit_transpile)
    assert np.allclose(cirq_u, cirq_transpile_u)
    # assert np.allclose(braket_u, braket_transpile_u)  # TODO
...
def test_yes_braket_no_qiskit(gate_str):
    ...
    braket_u, qbraid_braket_circ = braket_gate_test_circuit(braket_gate, nqubits)
    qiskit_circuit = transpile(qbraid_braket_circ, "qiskit")
    qiskit_u = to_unitary(qiskit_circuit)
    assert np.allclose(braket_u, qiskit_u)


@pytest.mark.skip(reason="test not compat")
...
def test_yes_qiskit_no_braket(gate_str):
    ...
    qiskit_u, qbraid_qiskit_circ = qiskit_gate_test_circuit(qiskit_gate, nqubits)
    braket_circuit = transpile(qbraid_qiskit_circ, "braket")
...
def test_yes_qiskit_no_cirq(gate_str):
    ...
    qiskit_gate = qiskit_init_gate(*params)
    qiskit_u, qbraid_qiskit_circ = qiskit_gate_test_circuit(qiskit_gate, nqubits)
    cirq_circuit = transpile(qbraid_qiskit_circ, "cirq")
...
def test_yes_braket_no_cirq(gate_str):
    ...
    braket_gate = braket_init_gate(*params)
    braket_u, qbraid_braket_circ = braket_gate_test_circuit(braket_gate, nqubits)
    cirq_circuit = transpile(qbraid_braket_circ, "cirq")
    cirq_u = to_unitary(cirq_circuit)
...
def test_yes_cirq_no_qiskit(gate_str):
    ...
    cirq_u, qbraid_cirq_circ = cirq_gate_test_circuit(cirq_gate, nqubits)
    qiskit_circuit = transpile(qbraid_cirq_circ, "qiskit")
    qiskit_u = to_unitary(qiskit_circuit)
    assert np.allclose(cirq_u, qiskit_u)
...
def test_yes_cirq_no_braket(gate_str):
    ...
    if gate_str == "U3":
        ...
        cirq_gate = cirq_init_gate(exponent=exp)
    cirq_u, qbraid_cirq_circ = cirq_gate_test_circuit(cirq_gate, nqubits)
    braket_circuit = transpile(qbraid_cirq_circ, "braket")
    braket_u = to_unitary(braket_circuit)
    assert np.allclose(cirq_u, braket_u)
...
def test_braket_transpile_ccnot():
    ...
    qbraid_braket = convert_to_cirq(braket_circuit)
    qiskit_circuit = transpile(qbraid_braket, "qiskit")
...
def test_1000_random_circuits(num_qubits):
    for _ in range(10):
        ...
        target_circuit = transpile(origin_circuit, target)
...
def test_non_contiguous_qubits_braket():
    ...
    qbraid_wrapper = convert_to_cirq(braket_circuit)
    cirq_circuit = transpile(qbraid_wrapper, "cirq")
    qiskit_circuit = transpile(qbraid_wrapper, "qiskit")
...
def test_non_contiguous_qubits_cirq():
    ...
    qbraid_wrapper = convert_to_cirq(cirq_circuit)
    qiskit_circuit = transpile(qbraid_wrapper, "qiskit")
    braket_circuit = transpile(qbraid_wrapper, "braket")
    cirq_unitary = to_unitary(cirq_circuit)
    qiskit_unitary = to_unitary(qiskit_circuit)
    braket_unitary = to_unitary(braket_circuit)
...
def test_non_contiguous_qubits_qiskit():
    ...
    qbraid_wrapper = convert_to_cirq(qiskit_circuit)
    braket_circuit = transpile(qbraid_wrapper, "braket")
    cirq_circuit = transpile(qbraid_wrapper, "cirq")
