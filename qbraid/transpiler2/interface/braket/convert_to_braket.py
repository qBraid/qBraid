...
from qbraid.transpiler2.utils import _int_from_qubit, _convert_to_contiguous_cirq
...
def to_braket(circuit: Circuit) -> BKCircuit:
    ...
    """
    compat_circuit = _convert_to_contiguous_cirq(circuit)
    ...
    if nqubits == 1:
        ...
        if op == cirq_ops.TOFFOLI.on(*op.qubits):
            ...
            try:
                ...
                return [
                    BKInstruction(
                        braket_gates.Unitary(matrix, display_name=name), qubits
                    )
                ]
            except:
