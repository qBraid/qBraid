...
def _raise_braket_to_cirq_error(instr: Any) -> None:
    raise ValueError(f"Unable to convert the instruction {instr} to Cirq.")
...
def from_braket(circuit: BKCircuit) -> Circuit:
    ...
    """
    compat_circuit = _convert_to_contiguous_braket(circuit)
    BK_qubits = [int(q) for q in compat_circuit.qubits]
    cirq_qubits = list(reversed([LineQubit(x) for x in range(len(BK_qubits))]))
