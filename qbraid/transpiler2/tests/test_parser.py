...
def test_unsupported_format():
    ...
    with pytest.raises(
        QasmException, match="Unsupported.*2.1.*2.0.*supported.*"
    ):
        parser.parse(qasm)
...
def test_error_not_starting_with_format(qasm: str):
    ...
    with pytest.raises(
        QasmException, match="Missing 'OPENQASM 2.0;' statement"
    ):
        parser.parse(qasm)
...
"""
    ...
    with pytest.raises(
        QasmException, match=r"Non matching.*length \[2 3\].*line 4"
    ):
        parser.parse(qasm)
...
"""
    ...
    with pytest.raises(
        QasmException, match=r"Out of bounds.*4.*q1.*2.*line 4"
    ):
        parser.parse(qasm)
...
"""
    ...
    ct.assert_allclose_up_to_global_phase(
        cirq.unitary(parsed_qasm.circuit),
...
"""
    ...
    with pytest.raises(
        QasmException, match=r".*not recognized.*'nonexistent'.*line 3"
    ):
        parser.parse(qasm)
...
def test_rotation_gates(qasm_gate: str, cirq_gate: cirq.SingleQubitGate):
    ...
    expected_circuit = Circuit()
    expected_circuit.append(
        cirq.Moment([cirq_gate(np.pi / 2).on(q0), cirq_gate(np.pi).on(q1)])
    )
    expected_circuit.append(
        cirq.Moment(
            [
...
def test_qelib_gate_without_include_statement():
    ...
    with pytest.raises(
        QasmException, match=r"""Unknown gate "x".* line 3.*forget.*\?"""
    ):
        parser.parse(qasm)
...
def test_measure_mismatched_register_size():
    ...
    with pytest.raises(
        QasmException, match=r""".*mismatched .* 2 -> 3.*line 5"""
    ):
        parser.parse(qasm)
...
def test_measure_to_quantum_register():
    ...
    with pytest.raises(
        QasmException, match=r"""Undefined classical register.*q1.*line 6"""
    ):
        parser.parse(qasm)
...
def test_measure_undefined_classical_bit():
    ...
    with pytest.raises(
        QasmException, match=r"""Undefined classical register.*c2.*line 5"""
    ):
        parser.parse(qasm)
...
def test_measure_from_classical_register():
    ...
    with pytest.raises(
        QasmException, match=r"""Undefined quantum register.*c1.*line 6"""
    ):
        parser.parse(qasm)
...
"""
    ...
    with pytest.raises(
        QasmException, match=r"Out of bounds bit.*4.*c1.*size 3.*line 4"
    ):
        parser.parse(qasm)
...
"""
    ...
    expected_circuit = Circuit()
    expected_circuit.append(cirq.ops.ZPowGate(exponent=1.0 / 3.0)(q0))
...
"""
    ...
    expected_circuit = Circuit()
    expected_circuit.append(
        cirq_qasm_gates.U2Gate(2.0 * np.pi, np.pi / 3.0)(q0)
    )
...
"""
    ...
    expected_circuit.append(
        cirq.Moment([cirq_qasm_gates.U3Gate(3.14, -np.pi, 8.0)(q0)])
    )
...
"""
    ...
    with pytest.raises(
        QasmException,
        match=r".*{}.* takes {}.*got.*5.*line 4".format(qasm_gate, num_params),
    ):
        parser.parse(qasm)
    ...
    with pytest.raises(
        QasmException,
        match=r".*{}.* takes {}.*got.*0.*line 4".format(qasm_gate, num_params),
    ):
        parser.parse(qasm)
...
def test_three_qubit_gates(
    qasm_gate: str, cirq_gate: cirq.testing.TwoQubitGate
):
    qasm = """
     OPENQASM 2.0;
     include "qelib1.inc";
...
"""
    ...
    with pytest.raises(
        QasmException,
        match=r""".*{}.* takes 3 arg\(s\).*got.*1.*line 4""".format(qasm_gate),
    ):
        parser.parse(qasm)
...
"""
    ...
    with pytest.raises(
        QasmException, match=r""".*{}.*parameter.*line 4.*""".format(qasm_gate)
    ):
        parser.parse(qasm)
