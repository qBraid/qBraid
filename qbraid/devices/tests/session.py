...
def _get_qbraid_config():
    """Returns a dictionary containing email, token and url from qbraidrc."""
    section = "default"
    config_dict = {"url": None, "email": None}
    qbraidrc_path = f"{os.environ.get('HOME')}/.qbraid/qbraidrc"
    if os.path.isfile(qbraidrc_path):
        config = configparser.ConfigParser()
        config.read(qbraidrc_path)
        if section in config.sections():
            for config_name in config[section]:
                config_dict[config_name] = config[section][config_name]
    return config_dict


CLIENT_APPLICATION = _get_client_header()
QBRAID_CONFIG = _get_qbraid_config()
...
class RetrySession(Session):
    ...
    def _add_qbraid_headers(self):
        self.headers.update({'email': QBRAID_CONFIG['email']})
        if 'id-token' in QBRAID_CONFIG:
            self.headers.update({'id-token': QBRAID_CONFIG['id-token']})
        elif 'refresh-token' in QBRAID_CONFIG:
            self.headers.update({'refresh-token': QBRAID_CONFIG['refresh-token']})
        else:
            raise AttributeError("Missing required qbraidrc value 'id-token' or 'refresh-token'")
