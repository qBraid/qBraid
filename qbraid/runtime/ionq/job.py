...
class IonQJob(QuantumJob):
    ...
    @staticmethod
    def _num_qubits(result: dict[str, Any]) -> int | None:
    ...
    @staticmethod
    def _rekey_to_bitstrings(meas_prob: dict[str, float], num_qubits: int | None) -> MeasProb:
        """Rekey a histogram from IonQ's decimal state indices to fixed-width bitstrings.

        Qubit 0 is the least significant bit of IonQ's index, so plain binary formatting
        already puts it last, matching every other qBraid route. Without ``num_qubits``
