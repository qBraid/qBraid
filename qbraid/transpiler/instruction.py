"""InstructionWrapper Class"""

from qbraid.transpiler._utils import instruction_outputs
from qbraid.transpiler.transpiler import QbraidTranspiler


class InstructionWrapper(QbraidTranspiler):
    """Abstract class for instruction objects. Instructions are objects which store a quantum
    gate and the qubits applied to that gate. These objects may be named differently in various
    pacakges, but their function is the same.

    Attributes:
        params (list): return abstract params list as :class:`qbraid.transpiler.paramater.ParamID`
