...
def create_gateset_qasm(max_operands) -> jnp.ndarray:
    ...
    gates = jnp.array(
        gates, dtype=[("gate", object), ("num_qubits", jnp.int64), ("num_params", jnp.int64)]
    )
    return gates
...
def _qasm3_random(
    ...
    num_qubits = jax.random.randint(key, (1,), 1, 4) if num_qubits is None else num_qubits
    depth = jax.random.randint(key, (1,), 1, 4) if depth is None else depth
    max_operands = jax.random.randint(key, (1,), 1, 3) if max_operands is None else max_operands
    try:
        if seed is None:
            seed = jnp.random.randint(0, jnp.iinfo(jnp.int32).max)
...
"""
        ...
        qubits = jnp.arange(num_qubits)
        gates = create_gateset_qasm(max_operands)
        for _ in range(depth):
            gate_specs = rng.choice(gates, size=num_qubits)
            cumulative_qubits = jnp.cumsum(gate_specs["num_qubits"], dtype=jnp.int64)

            max_index = jnp.searchsorted(cumulative_qubits, num_qubits, side="right")
            ...
            if slack:
                ...
                gate_specs = jnp.hstack((gate_specs, slack_gates))

            q_indices = jnp.empty(len(gate_specs) + 1, dtype=jnp.int64)
            p_indices = jnp.empty(len(gate_specs) + 1, dtype=jnp.int64)
            q_indices[0] = p_indices[0] = 0
            jnp.cumsum(gate_specs["num_qubits"], out=q_indices[1:])
            jnp.cumsum(gate_specs["num_params"], out=p_indices[1:])
            parameters = rng.uniform(0, 2 * jnp.pi, size=p_indices[-1])
