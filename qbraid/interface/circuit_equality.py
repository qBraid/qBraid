...
# a = np.random.rand(5000, 5000) + 1j * np.random.rand(5000, 5000)
# b = np.random.rand(5000, 5000) + 1j * np.random.rand(5000, 5000)

# a_jax = jnp.array(a)
# b_jax = jnp.array(b)

# # Initial run to include JIT compilation time
# start = time.perf_counter()
# result_a, result_b = match_global_phase(a_jax, b_jax)
# jax.block_until_ready(result_a)
# jax.block_until_ready(result_b)
# print(f"JAX Initial Time taken: {time.perf_counter() - start:.4f} seconds")

# # Subsequent runs to measure performance after JIT compilation
# start = time.perf_counter()
# for _ in range(10):
#     result_a, result_b = match_global_phase(a_jax, b_jax)
#     jax.block_until_ready(result_a)
#     jax.block_until_ready(result_b)
# print(f"JAX Subsequent Time taken (average over 10 runs): {(time.perf_counter() - start) / 10:.4f} seconds")
