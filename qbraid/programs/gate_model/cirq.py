...
class CirqCircuit(GateModelProgram):
    ...
    def _make_qubits(qubits: list[cirq.Qid], targets: list[int]) -> list[cirq.Qid]:
        if len(set(type(qubit) for qubit in qubits)) > 1:
            ...
            qubit_type = type(qubits[0])  # type: ignore[assignment]

        if qubit_type == cirq.LineQubit:
            return [cirq.LineQubit(i) for i in targets]
        if qubit_type == cirq.GridQubit:
            rows = [qubit.row for qubit in qubits]  # type: ignore[attr-defined]
            cols = [qubit.col for qubit in qubits]  # type: ignore[attr-defined]
    ...
    def align_final_measurements(circuit: cirq.Circuit) -> cirq.Circuit:
        ...
        last_ops: dict[cirq.Qid, cirq.Operation | None] = {
            qubit: None for qubit in circuit.all_qubits()
        }

        for moment in circuit:
            for op in moment.operations:
                if isinstance(op.gate, cirq.MeasurementGate):
                    last_ops[op.qubits[0]] = op  # type: ignore[assignment]

        if all(op is not None for op in last_ops.values()):
            measurement_moment = cirq.Moment()
            for op in last_ops.values():  # type: ignore[assignment]
                measurement_moment = measurement_moment.with_operation(op)  # type: ignore[arg-type]
            for moment in circuit:
