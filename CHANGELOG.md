...
### Fixed
- Fixed `GroupJobSession` failing to authenticate whenever the API key is passed in code rather than configured on the machine. It built its default client with no credentials, so a session opened alongside `QbraidProvider(api_key=...)` raised `AuthError`. It now takes an `api_key` of its own, mutually exclusive with `client`. Note that job groups also need [qbraid-core#315](https://github.com/qBraid/qbraid-core/pull/315), without which every group response fails validation ([#1382](https://github.com/qBraid/qBraid/pull/1382))
