...
- Conversion weights are corrected and documented now that they drive routing (#1312): `qiskit_to_qasm2` returns to 1.0 (its 0.999 came from an unrelated 2025 PR and matched no benchmark), and `cirq_to_pyquil` stays at 0.74 with its rationale recorded -- routing through `qasm2` measures ~0.95 gate fidelity versus ~0.88 direct. A new measured-circuit benchmark (`tests/transpiler/test_measurement_coverage.py`) sweeps every reachable conversion pair with a measured GHZ circuit and pins the readout-register shape for pyQuil targets, closing the structural blind spot of the gate-only coverage suites. Stale coverage floors raised to match measured accuracy: `cirq -> pyquil` 0.74 to 0.85, `pyquil -> *` 0.77 to 0.83-0.86, `braket -> pyquil` 0.84 to 0.95 ([#1313](https://github.com/qBraid/qBraid/pull/1313))
- `transpile()` now selects conversion paths by declared conversion weight rather than hop count alone. `find_top_shortest_conversion_paths` sorted candidates by `len`, so `edge["weight"]` was written but never read on the path `transpile()` takes, and the five conversions carrying reduced weights never influenced routing. Paths are now ranked by summed conversion cost, with hop count and node aliases as tie-breakers so routing no longer depends on the order conversions were registered in. `find_shortest_conversion_path` ranks by the same key, so it can no longer report a path `transpile()` would not take. Both finders also stop enumerating every simple path, which is factorial in graph density and made a dense 11-node graph take seconds per call: the single-path search uses Dijkstra and top-N selection uses Yen's algorithm, so a dense 18-node graph answers in under a millisecond. Most visibly, `cirq -> pyquil` now routes via `qasm2` instead of taking the direct weight-0.74 conversion, which splits the readout register into one `BIT[1]` per measurement key ([#1307](https://github.com/qBraid/qBraid/issues/1307))
...
- Fixed `RigettiJob` silently corrupting measurement bitstrings for programs declaring multiple readout registers: registers were concatenated in lexicographic order, not the program's `DECLARE` order, so a program declaring `ro` then `r1` got every bitstring's halves swapped with no error (confirmed in production). Registers now concatenate in the submitted program's `DECLARE` order, recovered from the compiled program stored at submission. Multi-register jobs rehydrated by ID, where that program is unavailable, raise a clear error instead of guessing. Single-register jobs are unaffected ([#1308](https://github.com/qBraid/qBraid/issues/1308))
- Measurement instructions now survive conversion between every pair of program types covered by the new measured-circuit benchmark (braket, cirq, pyquil, pytket, qiskit, and the qasm2/qasm3/openqasm3 dialects). `cirq_to_braket` converts `MeasurementGate`s to Braket `Measure` instructions instead of silently dropping them (mid-circuit measurement raises `ProgramConversionError`, since Braket cannot represent it); `pytket_to_braket` re-appends the measurements that pytket's `tk_to_braket` discards; `braket_to_cirq` collects Braket's keyless terminal measurements into one keyed `MeasurementGate` instead of per-qubit `M('')` ops that downstream exports collapse; and `cirq_to_pyquil` merges terminal measurements into a single readout register instead of declaring one `BIT[1]` register per measurement key -- the fragmented form QCS rejects. The measured-circuit benchmark's known-failure list is now empty ([#1313](https://github.com/qBraid/qBraid/pull/1313))
- Fixed `transpile(max_path_depth=...)` reporting no conversion path when the top `max_path_attempts` candidates all exceeded the depth limit, even though a path within it ranked just below them. The limit is now applied while selecting paths rather than to the already-truncated list, and `find_top_shortest_conversion_paths` takes a `max_depth` argument to that end ([#1312](https://github.com/qBraid/qBraid/pull/1312))
- Fixed `cirq_to_braket` writing every measurement to classical bit 0, so a converted circuit returned one qubit's outcome repeated instead of the register. Braket's `Measure` defaults its index to 0 and only `Circuit.measure()` assigns them, so building the instructions directly needs the index supplied explicitly; each measurement now gets its own bit ([#1312](https://github.com/qBraid/qBraid/pull/1312))
- Fixed `cirq_to_pyquil` assigning merged readout bits in operation order rather than by measurement key, transposing results whenever a higher-indexed qubit was measured in an earlier moment. `measure q[2] -> c[0]` now lands q_2 at bit 0 regardless of moment order. Merging also no longer reuses a measurement key already present in the circuit (which produced a circuit cirq rejects on simulation), and a terminal measurement carrying a `confusion_map` raises `ProgramConversionError` rather than having its readout error model silently dropped ([#1312](https://github.com/qBraid/qBraid/pull/1312))
- Made weighted path selection exact for user-supplied conversion graphs, where adversarial fuzzing against the enumerate-and-sort definition found two divergences (the shipped native graph was unaffected by both). Zero-weight conversions now carry a large finite cost instead of `float("inf")` -- `inf + x == inf` broke the order-preservation Dijkstra and Yen's rely on, so even `shortest_path()` could return a route the ranking placed second; paths through zero-weight edges now rank by fewest such edges first. And each Yen's spur search is seeded with its deviation root's ranking key, so every cost is the same left-to-right float sum -- previously, two paths whose costs were equal in exact arithmetic could differ in the last ulp between the spur search and the final ranking, emitting top-N paths out of order. Also: a negative `bias`, which makes edge costs negative and silently voids the search's correctness, is now rejected with a `ValueError`, and `top_n <= 0` reports no path found instead of returning one ([#1312](https://github.com/qBraid/qBraid/pull/1312))
- Fixed `braket_to_cirq` ordering the merged readout register by instruction encounter order rather than by each `Measure`'s classical bit index, which transposed results for circuits with permuted mappings -- e.g. `Circuit.from_ir` on OpenQASM measuring `b[2] = measure q[0]`. Braket's own simulator honors the index; the converter now sorts by it, falling back to instruction order for directly built `Measure`s, which default to index 0 ([#1312](https://github.com/qBraid/qBraid/pull/1312))
- Fixed `pytket_to_braket` silently dropping a classical bit when a qubit is measured into two different bits: the mid-circuit guard only caught gates after a measurement, not a second measurement, and the re-added measures are keyed by qubit. It now raises `ProgramConversionError` like every other mid-circuit measurement shape. The reverse case — two qubits measured into the same classical bit, which Braket would silently widen into two result bits — is rejected the same way ([#1312](https://github.com/qBraid/qBraid/pull/1312))
- The edge-cost transform uses `-log(w)` instead of `log(1/w)`: the reciprocal overflows to infinity for subnormal weights (below ~1e-308), which would rank a valid tiny positive weight *worse* than weight 0 and reintroduce the infinite-cost pathology the finite zero-weight cost exists to avoid ([#1312](https://github.com/qBraid/qBraid/pull/1312))
- `max_path_depth` now bounds the path search itself (each search carries a node-count budget, settling per `(node, hops)` state) rather than filtering results afterward. Previously a depth-capped query with no qualifying path sent Yen's algorithm enumerating every simple path in the graph before reporting "no path" — over 20 seconds on a dense 12-node graph, now sub-millisecond ([#1312](https://github.com/qBraid/qBraid/pull/1312))
