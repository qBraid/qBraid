...
### Added
- Added `IQMProvider`, `IQMDevice`, `IQMJob`, and `IQMSession` classes implementing the qBraid runtime interface for [IQM Resonance](https://resonance.meetiqm.com). Registers `iqm-client`'s native `Circuit` as a program type with an `IQMProgram` wrapper, plus `qiskit -> iqm` and `cirq -> iqm` transpiler edges, so the `iqm` extra needs only `iqm-client` — Qiskit is an optional enhancement that enables topology routing, not a requirement for submitting. Conversions emit logical qubit names and `IQMDevice` binds them to physical qubits through `submit_circuits(qubit_mapping=...)` ([#1149](https://github.com/qBraid/qBraid/pull/1149))
