...
### Added
- Added `QdiProvider`, `QdiDevice`, and `QdiJob`, a transport-agnostic runtime provider for any device that speaks the [Quantum Device Interface (QDI)](https://github.com/shassinger/qdi-demo) v0.2. Point it at a QDI HTTP endpoint with `base_url`/`token`, or hand it any in-process QDI client (an instance, a `qbraid.qdi_clients` entry-point name such as `"oqtopus"`, or a `"module:Class"` path) plus the `credentials` its `authenticate` call expects. Device descriptors become `TargetProfile`s with `qasm2`/`qasm3` program specs, so any qBraid-supported program type can be run; `QdiDevice.estimate()` exposes QDI resource estimation, and `QdiJob.cancel()` raises because v0.2 defines no cancel operation ([#1396](https://github.com/qBraid/qBraid/pull/1396))
