...
### Improved / Modified
- `GroupJobSession`'s docstring now says where its default client gets credentials from, so it is clear that a key given to `QbraidProvider(api_key=...)` has to be handed over as `client=provider.client`. Its context-manager example also called `results.items()`, which raises `AttributeError` because `GroupResult` exposes the mapping as `.results`
