...
### Fixed
- A list of circuits submitted to `QPerfectDevice` without an explicit `algorithm` now runs as an MPS batch instead of raising `ValueError`, matching what MIMIQ's `auto` selects for a single circuit. Pass `algorithm="statevector"` to keep exact simulation ([#1428](https://github.com/qBraid/qBraid/pull/1428))
