...
### Improved / Modified
- Conversion weights are corrected and documented now that they drive routing (#1312): `qiskit_to_qasm2` returns to 1.0 (its 0.999 came from an unrelated 2025 PR and matched no benchmark), `pytket_to_braket` drops to 0.95 because pytket's `tk_to_braket` discards measurement instructions (restoring `qasm2 -> braket` to its measurement-preserving route via `qasm3`, and fixing `pytket -> qasm3`, which lost measurements on `main`), and `braket_to_cirq` keeps 0.99 with its real rationale recorded -- braket measurements become empty-key cirq `M('')` ops that collapse downstream. A new measured-circuit benchmark (`tests/transpiler/test_measurement_coverage.py`) sweeps every reachable conversion pair with a measured GHZ circuit and pins the readout-register shape for pyQuil targets, closing the structural blind spot of the gate-only coverage suites. Stale coverage floors raised to match measured accuracy: `cirq -> pyquil` 0.74 to 0.85, `pyquil -> *` 0.77 to 0.83-0.86, `braket -> pyquil` 0.84 to 0.95 ([#1313](https://github.com/qBraid/qBraid/pull/1313))
