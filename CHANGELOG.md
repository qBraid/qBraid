...
### Improved / Modified
- README header and runtime diagram now follow the reader's GitHub theme, using the same light/dark asset pairs as the SDK docs. The header lockup is cropped to its content, so it no longer renders small inside a mostly-empty canvas
