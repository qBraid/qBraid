...
### Added
- Added `AQTProvider`, `AQTDevice`, `AQTJob`, and `AQTSession` classes implementing the qBraid runtime interface for AQT (Alpine Quantum Technologies) arnica-cloud devices. The device profile targets the existing `qiskit` alias with a `serialize` hook (`qiskit_to_aqt`) that delegates to `qiskit-aqt-provider` to build the AQT `quantum_circuit` payload, so any qBraid-supported program is transpiled to qiskit and then serialized without registering a new transpiler program type. Authentication is resolved non-interactively via `aqt-connector` (from `AQT_ACCESS_TOKEN`, or `AQT_CLIENT_ID`/`AQT_CLIENT_SECRET` client credentials), and job I/O hits the arnica v1 REST API directly. Devices are addressed by a `"<workspace>/<resource>"` `device_id`. Available via the new `aqt` optional-dependency extra ([#1262](https://github.com/qBraid/qBraid/pull/1262))
