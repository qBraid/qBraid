...
### Fixed
- Fixed IBM API failures being indistinguishable from one another: `QiskitRuntimeProvider._ibm_api_get` and `._exchange_api_key` collapsed every error into `ValueError("IBM API request failed: ...")`, discarding the HTTP status, so callers had to string-match the message (downstream, this made qBraid Lab tell users with valid credentials to re-save them when a job ID simply didn't exist). They now raise typed exceptions carrying `status_code`, exported from `qbraid.runtime`: `JobNotFoundError` (404), `AuthorizationError` (401/403), and `RuntimeAPIError` (other). The exceptions also carry IBM's own `error_code`, request `trace`, `solution` and `more_info` from its `ErrorContainer` response body. The error code is finer-grained than the HTTP status (e.g. `1291` job not found, `1200` not authorized), and `solution` is IBM's user-facing remediation text — note it is a suggestion, not gospel: it is absent from IBM's published schema, may contain Markdown, and its 401 text still recommends the retired `IBMProvider.save_account`. A 401/403 whose body is *not* an `ErrorContainer` — i.e. one produced by an edge proxy in front of the API rather than by IBM — is reported as a plain `RuntimeAPIError`, since it says nothing about whether the caller's credentials are valid. Backwards compatible — `RuntimeAPIError` subclasses `ValueError` and `JobNotFoundError` subclasses `ResourceNotFoundError`, so existing handlers keep working; the `ValueError` base is transitional and may be dropped in a future major release ([#1258](https://github.com/qBraid/qBraid/pull/1258))
