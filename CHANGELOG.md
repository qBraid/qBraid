...
### Fixed
- Fixed OpenQASM programs being rejected before submission for AWS- and Open Quantum-hosted IonQ devices when they used constructs IonQ JSON cannot express, a Braket verbatim box above all. Both vendors forward the program as written, so the IonQ JSON check no longer applies to them
