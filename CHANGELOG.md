...
### Added
- Added cross-repo integration test workflow (`.github/workflows/cross-repo-test.yml`) and report script (`.github/scripts/parse_test_report.py`) to support testing the qBraid SDK against in-development branches of `qbraid-core` and `pyqasm` before they are released ([#PLACEHOLDER](https://github.com/qBraid/qBraid/pull/PLACEHOLDER))
