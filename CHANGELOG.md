...
### Improved / Modified
- `QuantinuumDeviceError` and `QuantinuumJobError` moved to a public `qbraid.runtime.quantinuum.exceptions` module and are now exported from `qbraid.runtime.quantinuum`. The old import paths (`.device`, `.job`, `._transport`) still work ([#1297](https://github.com/qBraid/qBraid/pull/1297))
- Quantinuum jobs constructed with a cached terminal status (e.g. rehydrated from persisted metadata) now fetch NEXUS status details on demand in `execution_time_s`, instead of raising "status detail is missing" for a job that genuinely finished ([#1297](https://github.com/qBraid/qBraid/pull/1297))
...
- Fixed `QuantinuumJob.status()` never observing a job finish. It read a snapshot field on the job reference that is fixed when the reference is built, so `job = device.run(...)` followed by `job.result()` polled a submission-time status until it timed out. Status is now fetched from NEXUS on each call ([#1297](https://github.com/qBraid/qBraid/pull/1297))
- Fixed the Quantinuum job status map covering 5 of NEXUS's 10 states (a sixth key, `INITIALIZING`, is not a NEXUS status at all). `TERMINATED` and `DEPLETED` (credits exhausted) are terminal on NEXUS but mapped to the non-terminal `UNKNOWN`, so a stopped job was polled indefinitely; `SUBMITTED`, `CANCELLING` and `RETRYING` were also unmapped. A NEXUS status missing from the map now raises `QuantinuumJobError` instead of degrading to `UNKNOWN`, so the next unmapped state fails loudly rather than reproducing this bug ([#1297](https://github.com/qBraid/qBraid/pull/1297))
- Fixed Quantinuum QPUs whose name contains an "E" being reported as simulators. The check now matches the emulator/syntax-checker suffix rather than any occurrence of the letter, so `Helios-1` is correctly classified as hardware ([#1297](https://github.com/qBraid/qBraid/pull/1297))
...
- Fixed a timed-out Quantinuum execute dispatch escaping as a bare `httpx` exception with no way to locate the job NEXUS may have accepted. The dispatch is deliberately never retried (double-submit risk), so the error now names the project and the exact job name to search for and cancel ([#1297](https://github.com/qBraid/qBraid/pull/1297))
...
### Dependencies
- Constrained `qnexus` to `>=0.39.0,<0.48` in the `quantinuum` extra. Every 0.48.x release so far (0.48.0 through 0.48.2) imports `selene_core` without declaring it as a dependency, so `import qnexus` raises `ModuleNotFoundError` on a clean install; the Quantinuum tests now fail loudly in that state instead of silently skipping ([#1297](https://github.com/qBraid/qBraid/pull/1297))
