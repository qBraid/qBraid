...
- Fixed the Quantinuum job status map covering 6 of NEXUS's 10 states. `TERMINATED` and `DEPLETED` (credits exhausted) are terminal on NEXUS but mapped to the non-terminal `UNKNOWN`, so a stopped job was polled indefinitely; `SUBMITTED`, `CANCELLING` and `RETRYING` were also unmapped ([#1297](https://github.com/qBraid/qBraid/pull/1297))
...
- Fixed submits intermittently failing with `RemoteProtocolError: Server disconnected without sending a response`. NEXUS closes idle keep-alive connections, and qnexus never bounded its pool, so a request could reuse a socket the server had already closed. Idle connections now expire client-side first (`QUANTINUUM_NEXUS_KEEPALIVE_EXPIRY`, default 15s), turning the race into a cheap reconnect rather than a failed job. This complements the retry: a connection dropped before the request is sent is unambiguous, whereas retrying the execute stage risks double-submitting ([#1297](https://github.com/qBraid/qBraid/pull/1297))
