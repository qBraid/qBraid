...
### Fixed
- Fixed `BraketDevice.status()` raising `AttributeError: 'NoneType' object has no attribute 'service'` for an ONLINE device whose capabilities the installed `amazon-braket-schemas` cannot parse. Such a device now reports as `UNAVAILABLE` and emits a `UserWarning` naming it, so `run()` warns instead of crashing during validation
