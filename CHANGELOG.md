...
### Fixed
- Fixed `cirq_to_pyquil` raising `TypeError` when converting a `TwoQubitDiagonalGate` (e.g. a pyQuil `CPHASE00`/`CPHASE01`/`CPHASE10` round-tripped through cirq), whose diagonal angles are stored as a complex array: `exponent_to_pi_string` could not build a `Fraction` from a complex value. The angles are now coerced to their real part before formatting ([#PRNUM](https://github.com/qBraid/qBraid/pull/PRNUM))
