...
### Fixed
- Fixed `@cached_method` raising `TypeError: unhashable type` when a decorated method was called with unhashable arguments (e.g. `list`/`dict`), which blocked calls such as `BraketProvider.get_devices(statuses=["ONLINE", "OFFLINE", "RETIRED"])`. The decorator previously routed each call through an inner `functools.lru_cache`, which re-hashes the raw arguments; it now caches solely against the existing JSON-derived key (`_generate_cache_key`), so unhashable arguments are supported. TTL behavior, `maxsize` bounding, `cache_clear`, and `cache_info().currsize` are preserved, and the now-redundant `_QBRAID_TEST_CACHE_CALLS` test hook is removed ([#1244](https://github.com/qBraid/qBraid/pull/1244))
...
- Fixed `@cached_method` cache keys omitting instance identity: `_generate_cache_key` hashed only the class name (plus method/args), so two instances of the same provider class configured with *different* credentials shared cache entries within the TTL — one provider could be served another's cached results. The key now includes `hash(instance)` (providers hash their credentials, so equal credentials still share the cache), falling back to `id(instance)` for unhashable classes that define `__eq__` without `__hash__` ([#1244](https://github.com/qBraid/qBraid/pull/1244))
