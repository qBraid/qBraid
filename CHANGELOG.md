...
### Fixed
- **Breaking.** Measurement bitstrings now put qubit 0 last (little-endian, as Qiskit and IonQ already did) across providers. OQC and Rigetti direct results are reversed, so code reading those keys positionally must be updated. IonQ counts now pad to the submitted register width, so `x q[0]` on a six-qubit register returns `{"000001": N}` rather than `{"1": N}`. qBraid's native `qir-sv` simulator still reports qubit 0 first; that path is corrected server-side as part of the qBraid simulators refactor, not here ([#1327](https://github.com/qBraid/qBraid/pull/1327))
