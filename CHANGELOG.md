...
### Added
- Added `IQMProvider`, `IQMDevice`, `IQMJob`, and `IQMSession` classes implementing the qBraid runtime interface for [IQM Resonance](https://resonance.meetiqm.com). Registers `iqm-client`'s native `Circuit` as a program type with an `IQMProgram` wrapper, plus `qiskit -> iqm` and `cirq -> iqm` transpiler edges, so the `iqm` extra needs only `iqm-client` — Qiskit is an optional enhancement that enables topology routing, not a requirement for submitting. Conversions emit logical qubit names and `IQMDevice` binds them to physical qubits at submission. Devices are sized by the current calibration set rather than the chip, so a Star-architecture device such as Sirius reports the 16 qubits it can actually drive rather than all 24. `shots` defaults to 100, matching qBraid's other providers rather than `iqm-client`'s default of 1. Authenticate with `IQM_TOKEN`, or pass `token=` — `iqm-client` rejects the call if you supply both ([#1149](https://github.com/qBraid/qBraid/pull/1149))
