...
### Fixed
- Fixed `pyquil_to_cirq` on programs with a `DECLARE`d gate angle. The parameter now arrives as a sympy symbol, so `cirq.is_parameterized` sees it and the circuit can be printed, resolved, and simulated; previously all three failed ([#1358](https://github.com/qBraid/qBraid/pull/1358)).
