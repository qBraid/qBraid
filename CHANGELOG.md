...
- Fixed `openqasm3_to_pyquil` padding declared-but-idle qubits past the last one in use with identity, so a program using 2 of a declared 20 qubits asked the QPU for all 20. Only gaps between qubits in use are padded now ([#1296](https://github.com/qBraid/qBraid/pull/1301))
- Fixed `pyquil_to_cirq` rejecting `RXX`/`RYY`/`RZZ` as "noise gates". They now convert to Cirq's `XXPowGate`/`YYPowGate`/`ZZPowGate` ([#1296](https://github.com/qBraid/qBraid/pull/1301))
