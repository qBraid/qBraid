...
- Added `pennylane_to_qasm3` conversion, providing a direct transpiler edge from PennyLane `QuantumScript` to an OpenQASM 3.0 string. Uses `stdgates.inc` and covers single-qubit Clifford gates (H, X, Y, Z, S, T, SX and their adjoints sdg/tdg/sxdg), rotation gates (RX, RY, RZ, PhaseShift→p), two-qubit entangling gates (CX, CZ, SWAP, ControlledPhaseShift→cp, CRX/CRY/CRZ, IsingXX/YY/ZZ→rxx/ryy/rzz), three-qubit gates (Toffoli→ccx, CSWAP→cswap), and mid-circuit `Measure`/`Reset` operations. Raises `ValueError` on unsupported gates ([#1227](https://github.com/qBraid/qBraid/pull/1227))
