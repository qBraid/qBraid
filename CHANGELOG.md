...
### Fixed
- Fixed QASM 3 to QASM 2 conversion splitting complete terminal measurements into separate registers, which lost joint counts on OQC. QASM 3 to Cirq now gives a complete terminal register one key (`c`) rather than per-bit keys (`c_0`, `c_1`), so callers reading `cirq.Result.measurements` must update their lookups ([#1443](https://github.com/qBraid/qBraid/pull/1443)).
