...
- Fixed `delay` instructions being silently dropped by `qasm3_to_braket`, which produced a delay-free circuit with no error, warning, or log line. Braket has no gate-level delay — `braket.circuits.Circuit` cannot represent one, and `Circuit.from_ir` has no visitor registered for `DelayInstruction`, so the node is walked by the generic visitor and contributes no instructions. Circuits relying on idle time (T1/T2 characterization, dynamical decoupling) therefore ran as zero-delay circuits and returned meaningless data; verbatim was not a workaround, since the verbatim box is discarded on the same path. `qasm3_to_braket` now detects delays up front and raises `QasmError` naming the two ways to express idle time on hardware (`braket.pulse.PulseSequence.delay`, or Rigetti directly via Quil-T's `DELAY`). Raising is correct rather than conservative here: there is nothing to lower a delay to, and silently returning a circuit that runs but measures the wrong thing is worse than failing. Detection runs on the AST that `pyqasm.loads` already parses (via `original_program`), so it adds no extra parse and still catches delays inside constructs `pyqasm` cannot unroll, such as a verbatim box ([#1270](https://github.com/qBraid/qBraid/pull/1270))
