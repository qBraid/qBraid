...
### Dependencies
- Bumped the `pyqasm` floor to `>=1.0.4`, which includes the fix for the backend-dependent `dt` duration unit being silently relabeled as `ns` during unrolling ([pyqasm#317](https://github.com/qBraid/pyqasm/pull/317)). With `dt` now preserved, qBraid's OpenQASM 3 to pyQuil converter raises its `Unsupported delay unit: dt` error instead of emitting a silently wrong duration -- previously a Qiskit `delay(300, 0, unit="dt")` reached Quil as a 300 ns delay, corrupting timing experiments without any warning. Explicit `s`/`us`/`ns` delay units are unaffected ([#1285](https://github.com/qBraid/qBraid/pull/1285))
