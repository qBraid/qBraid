...
- **Breaking:** measurement bitstrings now place qubit 0 first (leftmost), matching Amazon Braket, OQC, Rigetti and CUDA-Q. Counts, probabilities and `measurements` arrays from `aws`, `aqt`, `ibm`, `ionq`, `openquantum`, `origin`, `quantinuum` and Azure's IonQ and Quantinuum paths are reversed relative to previous releases, and keys narrower than the register now widen to the right. `oqc`, `rigetti` and `qbraid` native devices already used this order and are unchanged. Two Azure paths are **not** covered and stay inconsistent until a follow-up: results in Microsoft's QIR format keep their existing register order, and Azure IonQ keys are still not padded to the submitted register width
...
### Fixed
- Fixed IonQ measurement keys dropping leading zeros, so `x q[0]` on a six-qubit register gave `{"1": N}` rather than `{"000001": N}`, and key width shifted with whichever outcomes a run happened to sample. Counts and probabilities are now padded to the submitted register width, matching every other qBraid route ([#1321](https://github.com/qBraid/qBraid/pull/1321))
