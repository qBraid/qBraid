...
- `transpile()` now selects conversion paths by declared conversion weight rather than hop count alone. `find_top_shortest_conversion_paths` sorted candidates by `len`, so `edge["weight"]` was written but never read on the path `transpile()` takes, and the five conversions carrying reduced weights never influenced routing. Paths are now ranked by summed conversion cost, with hop count and node aliases as tie-breakers so routing no longer depends on the order conversions were registered in. `find_shortest_conversion_path` ranks by the same key, so it can no longer report a path `transpile()` would not take; it reaches that path by Dijkstra rather than enumerating every simple path, which is factorial in graph density and made a dense 11-node graph take seconds per call. Most visibly, `cirq -> pyquil` now routes via `qasm2` instead of taking the direct weight-0.74 conversion, which splits the readout register into one `BIT[1]` per measurement key ([#1307](https://github.com/qBraid/qBraid/issues/1307))
