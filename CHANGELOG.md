...
### Fixed
- Fixed `RigettiDevice.run` failing for any program containing Quil-T (pulse/timing) instructions, which made idle-time experiments (T1/T2 characterization, dynamical decoupling) impossible on Rigetti QPUs. `transform()` sends every program to quilc, but quilc is a gate-model compiler and — per Rigetti's docs, "Quil-T instructions are not supported by quilc or the QVM" — cannot compile them: a `DELAY` fails with the opaque RPCQ error `The value #S(CL-QUIL.FRONTEND:CONSTANT :VALUE 5.0d-4 ...) is not of type CL-QUIL.FRONTEND:QUBIT` (the duration bound into a qubit slot), and `FENCE` fails to rewire. Programs using Quil-T now bypass quilc and are passed unchanged to the QCS translation service, which accepts both gate-model and pulse-model instructions. Because quilc is what would otherwise nativize the program, such programs must already use native gates (e.g. `RX(pi) 0` rather than `X 0`) — matching Rigetti's documented workflow, in which Quil-T programs are nativized by hand. Detection defers to pyquil's own `Program.remove_quil_t_instructions`, so what counts as Quil-T stays in sync with pyquil and covers `DEFCAL`/`DEFFRAME`/`DEFWAVEFORM` definitions. Gate-model programs are unaffected and still compiled by quilc ([#1271](https://github.com/qBraid/qBraid/pull/1271))
- Fixed the `RigettiDeviceError` raised on quilc compilation failure naming the two things that are never the cause, while omitting the one that is. It advised ensuring "the program is valid Quil and that the quilc compiler is running and accessible", but quilc's reachability is already established by the probe immediately above it, and it discarded quilc's own explanation — which only lived in the exception's `__cause__`, so callers persisting `str(exc)` (as job records do) were left with no reason at all. The message now surfaces quilc's error directly ([#1271](https://github.com/qBraid/qBraid/pull/1271))
