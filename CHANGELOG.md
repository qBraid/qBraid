...
- `transpile()` now selects conversion paths by declared conversion weight rather than hop count, so the reduced weights five conversions carry finally influence routing. Most visibly `cirq -> pyquil` now goes via `qasm2`, which measures ~0.95 gate fidelity against ~0.88 direct. Path search is also no longer factorial in graph density ([#1312](https://github.com/qBraid/qBraid/pull/1312))
