...
### Fixed
- Fixed `qasm2 -> pytket`, `pytket -> qasm2`, and `QuantinuumJob.compiled_program()` rejecting any circuit whose classical register is wider than 32 bits. All three took pytket's default `maxwidth=32`, so a measured 33-qubit circuit could not convert at all — on the path every Quantinuum device uses, against hardware that is already 56 qubits ([#PRNUM](https://github.com/qBraid/qBraid/pull/PRNUM))
