...
- Fixed `QuantinuumDevice.status()` raising `ResourceFetchFailed` (400, "Invalid machine name") for cloud-hosted NEXUS emulators such as `H1-Emulator`/`H2-Emulator`, which broke `device.run()` since the pre-submit validation calls `status()`. The NEXUS machine status endpoint only supports hardware-hosted devices, so the provider now records each device's `nexus_hosted` flag from the device list in its `TargetProfile`, and `status()` reports cloud-hosted devices as `ONLINE` without calling the endpoint (per qnexus, cloud-hosted emulators can be assumed always online)
