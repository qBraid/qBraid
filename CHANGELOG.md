...
### Added
- Added `qbraid.visualization.plot_connectivity_graph`, which renders a device's connectivity graph colored by live calibration data (edges by two-qubit gate error, nodes by readout error, darker is better, matching the qBraid Lab topology view). Lattice layouts are driven by the device document's `topology` config (`square-lattice` and `square-lattice-clipped`, with a rustworkx force-directed fallback for unknown types), and qubits with no working couplings are drawn as dotted outlines. Companion to `QbraidDevice.get_calibrations()`/`coupling_map` ([#1281](https://github.com/qBraid/qBraid/pull/1281)); the `lattice_positions` helper is exported for custom plots ([#1283](https://github.com/qBraid/qBraid/pull/1283))
...
### Fixed
- Fixed `openqasm3_to_pyquil` raising `AttributeError: 'str' object has no attribute 'name'` for any program addressing physical qubits (`x $0;`) rather than a declared register (`qubit[2] q; x q[0];`). `_flat_qubit` assumed every unrolled qubit reference is an `IndexedIdentifier`, but a physical qubit has no declaring register, so `pyqasm.unroll()` leaves it as a plain `Identifier` whose `name` is the literal string `"$0"` — the lookup then raised on the first gate. Physical qubit indices are now used verbatim (Quil is itself integer-indexed, so `$13` maps to pyQuil qubit `13`), and are deliberately not identity-padded, since such a program is already mapped to specific hardware qubits and renumbering would silently retarget it. This stranded a production `cepheus-1-108q` job at submission: a 16-qubit Qiskit-transpiled circuit already in Rigetti's native basis with 16 `delay` instructions, exactly the hand-mapped idle-time workload that [#1272](https://github.com/qBraid/qBraid/pull/1272) enabled. Every route failed at once, and the other two are upstream limitations rather than qBraid bugs: cirq's parser rejects `$`, and braket rejects `delay` ([#1270](https://github.com/qBraid/qBraid/pull/1270)) ([#1286](https://github.com/qBraid/qBraid/pull/1286))
- Fixed OpenQuantum `prepare_job` / `get_preparation_result` / `create_job` returning or failing without a clear message when the API rejected the request: responses are now parsed for the stable `type` field (from api-common `ApiErrorType`), mapping `TERMS_OF_USE_REQUIRED` to a `QbraidRuntimeError` that points users to https://www.openquantum.com to accept the Terms of Use, and other failures to a message that includes HTTP status, error type, and API detail (so e.g. `ORG_MEMBERSHIP_REQUIRED` is not mistaken for a terms issue)
- Fixed `RigettiJob.status()` silently discarding the reason a job failed: a non-timeout `QpuApiError` from `retrieve_results` (e.g. `job execution ID not found: <id>`) set the status to `FAILED` but neither logged the error nor preserved its message, so callers that persist failure reasons (such as job records) had nothing to store — ten production cepheus-1-108q jobs on 2026-07-21 were stranded `FAILED` with no `statusMsg` this way, with the actual QCS error unrecoverable after the fact. The error is now logged and its message recorded both on the returned status (`JobStatus.set_status_message`, matching the native and OQC providers) and on a new per-instance `RigettiJob.status_message` property, which is safe to read under concurrent polling where the shared enum member's message could be overwritten by another job ([#1278](https://github.com/qBraid/qBraid/pull/1278))
- Fixed `RigettiDevice.run` failing for any program containing Quil-T (pulse/timing) instructions, making idle-time experiments (T1/T2, dynamical decoupling) impossible on Rigetti QPUs. `transform()` sent every program to quilc, which is gate-model only — per Rigetti's docs, "Quil-T instructions are not supported by quilc or the QVM" — so a `DELAY` failed with an opaque `is not of type CL-QUIL.FRONTEND:QUBIT` error from the compiler. Quil-T programs now bypass quilc and go straight to the QCS translation service, which accepts pulse-model instructions. Since quilc is what would otherwise nativize them, such programs must already use native gates (`RX(pi) 0`, not `X 0`), matching Rigetti's documented hand-nativization workflow. Detection defers to pyquil's `Program.remove_quil_t_instructions`; gate-model programs are unaffected ([#1272](https://github.com/qBraid/qBraid/pull/1272))
- Fixed `RigettiDevice._submit` reporting every translation failure as `"Translation failed for quantum processor '<id>'. Ensure the program uses only native gates for the target QPU."`, discarding the translation service's own explanation into `__cause__` (leaving callers that persist `str(exc)`, such as job records, with no reason at all). The hint fits the most common cause but misdescribes the rest: translation also fails when `prepend_default_calibrations=False` leaves the program with no frames (`program has no defined frames`), when `passive_reset_delay_seconds` exceeds the QPU's 67.1 ms cap (`waveform duration ... exceeds maximum allowed duration`), and when a `DEFFRAME` differs from the Rigetti defaults — none of which are gate-nativity problems. Rigetti's message is also more precise where the hint did apply, naming the offending instruction (`at instruction 0 ("H 0"): this instruction must be replaced or decomposed prior to compilation`). The message now surfaces it ([#1275](https://github.com/qBraid/qBraid/pull/1275))
- Fixed the `RigettiDeviceError` raised on quilc compilation failure advising to ensure "the program is valid Quil and that the quilc compiler is running and accessible": quilc's reachability is already established by the probe immediately above it, and quilc's own explanation was discarded into `__cause__`, leaving callers that persist `str(exc)` (such as job records) with no reason at all. The message now surfaces quilc's error ([#1272](https://github.com/qBraid/qBraid/pull/1272))
- Improved error visibility for IBM Cloud IAM failures in `QiskitRuntimeProvider._exchange_api_key`: the raised `AuthorizationError`/`RuntimeAPIError` previously carried only the `HTTPError` status line (e.g. "Failed to exchange IBM API key: HTTP Error 400: Bad Request"), discarding the response body that explains the failure. The message now includes IAM's `errorCode`/`errorMessage`/`errorDetails` (e.g. `BXNIM0415E - Provided API key could not be found.`, the signature of an invalid or mis-copied API key), with `error_code` and IAM's `context.requestId` set on the exception's `error_code`/`trace` fields; non-JSON bodies are surfaced raw (truncated). Also hardened both IBM error-body parsers against streams that raise on read ([#1263](https://github.com/qBraid/qBraid/pull/1263))
- Fixed `QiskitRuntimeProvider` collapsing every IBM API failure into a bare `ValueError`, which forced callers to string-match messages (and led qBraid Lab to show credential guidance for job IDs that simply didn't exist). `_ibm_api_get` and `_exchange_api_key` now raise typed exceptions, exported from `qbraid.runtime`: `JobNotFoundError` (404), `AuthorizationError` (401/403), and `RuntimeAPIError` (other) — each carrying `status_code`, plus IBM's own `error_code`, `trace`, `solution`, and `more_info` when its `ErrorContainer` body is present. A 401/403 whose body is not an `ErrorContainer` (an edge proxy answered, not IBM) stays a plain `RuntimeAPIError`, since it says nothing about the caller's credentials. Backwards compatible: `RuntimeAPIError` subclasses `ValueError` (transitional; may be dropped in a future major) and `JobNotFoundError` subclasses `ResourceNotFoundError` ([#1258](https://github.com/qBraid/qBraid/pull/1258))
- Fixed two `@cached_method` caching bugs by removing the decorator's inner `functools.lru_cache` and caching solely against the JSON-derived key (`_generate_cache_key`): (1) methods called with unhashable arguments (e.g. `list`/`dict`) raised `TypeError: unhashable type`, blocking calls such as `BraketProvider.get_devices(statuses=["ONLINE", "OFFLINE", "RETIRED"])`; (2) the cache key omitted instance identity, so two providers of the same class with *different* credentials shared cache entries within the TTL — the key now includes `hash(instance)` (providers hash their credentials, so equal credentials still share the cache), falling back to `id(instance)` for unhashable classes. TTL behavior, `maxsize` bounding (oldest-by-insertion eviction), `cache_clear`, and `cache_info().currsize` are preserved; `maxsize=0` disables caching (matching `functools.lru_cache`), and cache reads/writes are guarded by a per-method lock ([#1244](https://github.com/qBraid/qBraid/pull/1244))
- Fixed `BraketProvider.get_devices` aborting the entire call with `QbraidError` when any returned device lacks a qBraid-supported program type (e.g. retired/legacy hardware such as D-Wave devices surfaced when `statuses` includes `"RETIRED"`). Such devices are now skipped (logged at info level) and the remaining devices are still returned ([#1244](https://github.com/qBraid/qBraid/pull/1244))

- Fixed `delay` instructions being silently dropped by `qasm3_to_braket`. Braket has no gate-level delay, and `Circuit.from_ir` discards `DelayInstruction` without raising, so circuits relying on idle time (T1/T2 characterization, dynamical decoupling) silently ran as zero-delay circuits — a verbatim box was no workaround, since it is dropped on the same path. `qasm3_to_braket` now detects delays and raises `QasmError` rather than returning a circuit that measures the wrong thing. Detection reuses the AST `pyqasm.loads` already parses (`original_program`), adding no extra parse while still catching delays in constructs `pyqasm` cannot unroll, such as a verbatim box ([#1270](https://github.com/qBraid/qBraid/pull/1270))
