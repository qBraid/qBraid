...
### Fixed
- `OQCDevice.status()` no longer reports a windowed QPU such as Toshiko as `UNAVAILABLE` while its access window is open. OQC only reports the next window, so status now comes from whether the device is active and its health checks pass ([#PR](https://github.com/qBraid/qBraid/pull/PR))
