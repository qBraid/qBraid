...
### Added
- `QudoraJob.map_status()` and `QudoraJob.parse_counts()` are now public, so a caller holding a job record can map its status and decode its histograms without fetching the record again. `status()` costs one request and `result()` three, which is worth avoiding against QUDORA's rate limit ([#PRNUM](https://github.com/qBraid/qBraid/pull/PRNUM))
