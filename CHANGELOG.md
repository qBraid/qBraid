...
- Not every change earns an entry. Write one when a reader could notice: behaviour they rely on changed, a bug they could have hit is fixed, or something new is available to them. Internal refactors, test-only changes, performance work with no visible effect, and the implementation details of a fix that is already described do not get their own entry -- fold them into the entry they belong to, or leave them to the PR.
...
### Added
- Added conversions between Pauli operators — Qiskit `SparsePauliOp`, OpenFermion `QubitOperator`, Cirq `PauliSum`, PennyLane `PauliSentence`, Amazon Braket `Observable` and CUDA-Q `SpinOperator` — with `transpile(op, "<alias>")`. Qubit `i` is the same qubit in every library, and operators with symbolic coefficients or no integer qubit index are refused rather than guessed at. Operators never convert to circuits. OpenFermion comes with the new `openfermion` extra ([#1448](https://github.com/qBraid/qBraid/pull/1448))
- Added `OQCJob.compiled_program()`, `OQCJob.execution_time_s()`, and `OQCJob.compile_time_s()`, exposing the circuit OQC actually compiled and the QPU and compilation times for a task. The compiled program reveals the physical qubits OQC selected. All three return `None` until OQC publishes the value, which happens once the task has compiled — before it is marked completed, not after. `OQCDevice.submit()` also accepts a `tag`, stored on the OQC task, which is the only way to attribute a task to an individual user when several share one OQC account ([#1420](https://github.com/qBraid/qBraid/pull/1420))
- Added `QuantinuumJob.compiled_program()`, returning the NEXUS-compiled program that ran on the QPU as a `Program` — `"qasm2"` (hqslib1 dialect) for circuit jobs, `"qir.bc"` for QIR jobs, a list for a batch. It reveals the native gates and qubit mapping Quantinuum's compiler chose, which measurement counts do not carry. Returns `None` for a job that has not completed ([#1410](https://github.com/qBraid/qBraid/pull/1410))
- Added a `job_properties` argument to `QuantinuumDevice.submit`/`run`, forwarded to NEXUS as job properties. NEXUS indexes them, so jobs sharing one project stay attributable — `qnx.jobs.get_all(properties={...})` filters by them. Each key must already be defined on the project via `qnx.projects.add_property`; NEXUS rejects undefined keys ([#1412](https://github.com/qBraid/qBraid/pull/1412))
...
- Added `QPerfectProvider`, giving access to QPerfect's MIMIQ cloud emulator through the new `qperfect` extra. Authenticate with `QPERFECT_USERNAME` and `QPERFECT_PASSWORD`, or with a `QPERFECT_API_TOKEN` refresh token — MIMIQ tokens expire after about a day, so credentials are the durable option. A single circuit runs with MIMIQ's `"auto"` algorithm; a batch defaults to `"mps"`, since MIMIQ does not accept `"auto"` for more than one circuit — pass `algorithm="statevector"` for exact simulation ([#1299](https://github.com/qBraid/qBraid/pull/1299), [#1332](https://github.com/qBraid/qBraid/pull/1332), [#1428](https://github.com/qBraid/qBraid/pull/1428))

### Improved / Modified
- An OQC task that has been accepted and is waiting to run, for example for the device's next access window, now reports `JobStatus.QUEUED` instead of `INITIALIZING` ([#1445](https://github.com/qBraid/qBraid/pull/1445))
- `GroupJobSession`'s docstring now says where its default client gets credentials from, so it is clear that a key given to `QbraidProvider(api_key=...)` has to be handed over as `client=provider.client`. Its context-manager example also called `results.items()`, which raises `AttributeError` because `GroupResult` exposes the mapping as `.results` ([#1390](https://github.com/qBraid/qBraid/pull/1390))
...
### Fixed
- `OQCDevice.get_next_window()` now raises `ResourceNotFoundError` for a device with no upcoming access window, such as an always-on simulator, instead of an `AttributeError`. The error now says whether OQC reported no window or the lookup failed ([#1438](https://github.com/qBraid/qBraid/pull/1438))
- `qbraid.runtime` no longer advertises `QirRunner`. The name was offered in tab completion but had not been exported since [#1175](https://github.com/qBraid/qBraid/pull/1175), so importing it raised `AttributeError` ([#1433](https://github.com/qBraid/qBraid/pull/1433))
- Fixed `qasm2 -> pytket`, `pytket -> qasm2`, and `QuantinuumJob.compiled_program()` rejecting a classical register wider than 32 bits, which is below H2-1's 56. The limit is now 64, pytket's own ceiling for exporting conditional circuits. `transpile()` already routed around the two converters via a longer path; calling them directly, and `compiled_program()`, did not ([#1432](https://github.com/qBraid/qBraid/pull/1432))
- Fixed `QbraidDevice.best_qubits()` ranking a qubit or edge whose published calibration error is negative as the best on the device, so circuits were routed onto corrupt data while being reported as the best chain. An error outside `[0, 1]` is now treated as unusable, matching how one at or above 1 already was ([#1429](https://github.com/qBraid/qBraid/pull/1429))
- Fixed `qasm3 -> ionq` and `qasm2 -> ionq` silently dropping a single-qubit gate whose register could not be resolved, an alias (`let a = q[0:1];`) being the clearest case. The converted circuit computed something different from the program with nothing to signal it; such a gate now raises `ValueError` naming the register ([#1424](https://github.com/qBraid/qBraid/pull/1424))
- Fixed `QuantumJob.async_result()` freezing the event loop on every status check. [#942](https://github.com/qBraid/qBraid/pull/942) moved the delay between polls onto `asyncio.sleep`, but each poll still called the synchronous `status()` directly on the loop, and `async_result` then called the equally synchronous `result()` — both issue HTTP requests on every provider. A job polled from a FastAPI service or a notebook kernel stalled everything else for the duration of each request; both calls now run in a worker thread ([#1423](https://github.com/qBraid/qBraid/pull/1423))
- Fixed `OQCJob.cancel()` raising `TypeError: OQCClient.cancel_task() got an unexpected keyword argument 'task_id'` for every OQC job. The client's parameter is `task_ids` ([#1420](https://github.com/qBraid/qBraid/pull/1420))
- Fixed every OpenQASM 2 program submitted to OQC failing to compile with `cannot use non-builtin custom instruction 'h' before definition`. `OQCDevice.transform()` stripped `include "qelib1.inc"`, which is where OQC's QASM 2 parser resolves the standard gates from, so QASM 2 is now passed through untouched ([#1420](https://github.com/qBraid/qBraid/pull/1420))
- Fixed OQC devices targeting `qasm3`, which made any circuit containing a two-qubit gate fail server-side with `argument of type 'NoneType' is not iterable`. OQC accepts both dialects, but its QASM 3 path currently rejects two-qubit gates, `cx` and `ecr` included. Devices now target `qasm2`, so a QASM 3 program is transpiled down rather than submitted and failed ([#1420](https://github.com/qBraid/qBraid/pull/1420))
- Fixed `qasm3 -> cirq` and `qasm2 -> cirq` making every statement after the first in a multi-statement `if` block unconditional. Each statement in the block is now classically controlled ([#1411](https://github.com/qBraid/qBraid/pull/1411))
- Fixed `BraketDevice.status()` raising `AttributeError: 'NoneType' object has no attribute 'service'` for an ONLINE device whose capabilities the installed `amazon-braket-schemas` cannot parse. Such a device now reports as `UNAVAILABLE` and emits a `UserWarning` naming it, so `run()` warns instead of crashing during validation ([#1407](https://github.com/qBraid/qBraid/pull/1407))
- Fixed `qasm2 -> ionq` and `qasm3 -> ionq` numbering every register's qubits from 0. In a program with more than one register, gates on later registers were sent to the wrong qubits ([#1405](https://github.com/qBraid/qBraid/pull/1405))
- Fixed `qasm3 -> ionq` crashing with `'Identifier' object has no attribute 'indices'` on circuits transpiled against a backend with `qiskit>=2`, which serializes physical qubits as `$0`. Hardware qubits now raise a clear error rather than crashing or, for single-qubit gates, being dropped silently. `QbraidDevice` validation errors for IonQ also now name the reason ([#1389](https://github.com/qBraid/qBraid/pull/1389))
- Fixed OpenQASM programs being rejected before submission for AWS- and Open Quantum-hosted IonQ devices when they used constructs IonQ JSON cannot express, a Braket verbatim box above all. Both vendors forward the program as written, so the IonQ JSON check no longer applies to them ([#1386](https://github.com/qBraid/qBraid/pull/1386))
- Fixed `Result.data.get_counts()` raising `TypeError` for Azure Pasqal jobs. Pasqal's emulators wrap the histogram as `{"counter": ..., "raw": ...}`, which was passed through instead of unwrapped, so no measurement counts were reachable ([#1381](https://github.com/qBraid/qBraid/pull/1381))
- Cirq and PyTKET circuits with unresolved gate parameters now raise `ProgramConversionError` naming those parameters when converting to Braket or OpenQASM 2, as do PyTKET conversions to PyQIR. Previously, Cirq conversions leaked numeric errors, while PyTKET-to-QASM 2 could emit invalid parameterized QASM. Symbols appearing only in a global phase are ignored, since the phase is discarded (`cirq -> braket` is unaffected: it rejects global phase outright, symbolic or not) ([#1373](https://github.com/qBraid/qBraid/pull/1373))
- **Breaking.** Measurement bitstrings now put qubit 0 last (little-endian, as Qiskit and IonQ already did) across providers. OQC and Rigetti direct results are reversed, so code reading those keys positionally must be updated. IonQ counts now pad to the submitted register width, so `x q[0]` on a six-qubit register returns `{"000001": N}` rather than `{"1": N}` ([#1327](https://github.com/qBraid/qBraid/pull/1327))
- Fixed `ConversionGraph(conversions=[])` building the full default graph instead of an edge-less one, so a filter that matched no conversions silently returned every registered one. An empty list is now honored, as are `reset([])` and `copy()` of a graph whose conversions were all removed ([#1371](https://github.com/qBraid/qBraid/pull/1371))
...
- Measurement instructions now survive conversion between the program types the new measured-circuit benchmark covers (braket, cirq, pyquil, pytket, qiskit, and the qasm2/qasm3/openqasm3 dialects). Four converters were silently corrupting readout: `cirq_to_braket` dropped measurements and wrote every result to bit 0, `cirq_to_pyquil` and `braket_to_cirq` transposed bit order, and `pytket_to_braket` dropped a classical bit ([#1313](https://github.com/qBraid/qBraid/pull/1313), [#1312](https://github.com/qBraid/qBraid/pull/1312))
- Fixed `transpile(max_path_depth=...)` reporting no conversion path when the top-ranked candidates all exceeded the limit, even though a qualifying path ranked just below. The limit now bounds the search itself, which also turns a depth-capped miss on a dense graph from over 20 seconds into sub-millisecond ([#1312](https://github.com/qBraid/qBraid/pull/1312))
- `cudaq_to_qasm2` now raises `ProgramConversionError` when `cudaq.translate` fails -- cudaq reports failure by returning the literal string `"{translation failed}"` with the error printed to stderr, which the transpiler previously passed along as a successful conversion instead of falling back to another path ([#1313](https://github.com/qBraid/qBraid/pull/1313))
...
### Dependencies
- Bumped the `amazon-braket-sdk` upper bound from `<1.127.0` to `<1.128.0`, so Python 3.11+ environments resolve `1.127.1`. Python 3.10 is unaffected: `amazon-braket-sdk` requires 3.11+ from `1.111.0` on, so 3.10 stays on `1.110.1` and `amazon-braket-schemas` `1.28.0` no matter where the cap sits ([#1406](https://github.com/qBraid/qBraid/pull/1406))
- Raised the `qbraid-core` floor from `>=0.3.9` to `>=0.6.1`. Job groups were unusable on every earlier release: `GroupJobSession.open()` raised `ValidationError` because `GroupJob` required `organizationUserId`, which the Runtime API no longer returns ([#1387](https://github.com/qBraid/qBraid/pull/1387))
