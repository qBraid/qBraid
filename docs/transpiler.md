...
Pauli operators (Qiskit `SparsePauliOp`, OpenFermion `QubitOperator`, Cirq `PauliSum`,
PennyLane `PauliSentence`, Braket `Observable`, CUDA-Q `SpinOperator`) are registered
program types that form their own component of the graph. No edge joins them to a
circuit, so `transpile()` never turns one into the other, and an operator can never be
submitted to a device.

- **Qubit `i` means the same physical qubit everywhere.** Each library's reader and writer
  in `conversions/_pauli_io.py` is the only place its convention lives (Qiskit labels are
  little-endian; Cirq uses `LineQubit(i)`; PennyLane and CUDA-Q use integer wires; Braket
  factors carry their own targets).
- **Refuse, never guess.** Symbolic coefficients, qubits without an integer index
  (`GridQubit`, string wires), untargeted Braket observables and non-Pauli terms raise
  `OperatorConversionError`.
- **Width is not preserved** by libraries that do not record it: trailing identity qubits
  are dropped, and writing back to Qiskit uses one past the highest qubit acted on.
- **Every library converts directly to every other.** The n × (n − 1) one-line conversions
  are generated by `bin/generate_operator_conversions.py`; add a library to its table, not
  by hand. Feeder aliases (Cirq `PauliString`, PennyLane operator arithmetic, Qiskit
  `SparseObservable`, CUDA-Q `SpinOperatorTerm`) have one hand-written edge into their
  library's canonical type.
...
| `_pauli_io.py`, an operator library, or its feeder | `python bin/generate_operator_conversions.py` (must be a no-op) and `pytest tests/transpiler/operators` |
